...
import com.cordestitch.service.service.pdfs.ReturnOrderService;
...
public class ReturnOrderServiceImplementation implements ReturnOrderService {
    ...
    @Override
    @Transactional
    public SuccessResponse sendReturnOrderEmail()  {
        try {
            ...
            attachmentPart.setFileName("Return_Orders_Report.pdf");
    ...
    public ByteArrayOutputStream generateByteArray() throws IOException {
        ...
        List<OrderItemEntity> orderedData = orderEntityList.stream()
                .flatMap(order -> order.getOrderItemEntities().stream())
                .filter(order -> "RETURNED".equalsIgnoreCase(String.valueOf(order.getDeliveryStatus())) || "DELIVERED".equalsIgnoreCase(String.valueOf(order.getDeliveryStatus())))
                .toList();
        ...
        dateParagraph.add("Returned Orders Report").setFont(companyFont);
        dateParagraph.add("\n");
        if (!orderedData.isEmpty()) {
            ...
            float[] bookingInfoColumnWidths = {120, 120, 120, 145, 120, 150, 150, 120, 130, 120};
            ...
            bookingTable.addCell(new Cell().add(createStyledCell("Return Reason", font)).setBackgroundColor(customGray).setOpacity(0.8f));
            bookingTable.addCell(new Cell().add(createStyledCell("Return Status", font)).setBackgroundColor(customGray).setOpacity(0.8f));
            ...
            for (OrderItemEntity orderItemEntity : orderedData) {
                OrderEntity entity = orderItemEntity.getOrderEntity();

                List <ReturnEntity> returnEntities =  returnRepository.findAll();
                for (ReturnEntity returnEntity : returnEntities) {
                    bookingTable.addCell(new Cell().add(createStyledCell(returnEntity.getOrderId(), font)));
                    bookingTable.addCell(new Cell().add(createStyledCell(returnEntity.getOrderItemId(), font)));
                    ...
                    bookingTable.addCell(new Cell().add(createStyledCell(returnEntity.getReturnReason(), font)));
                    bookingTable.addCell(new Cell().add(createStyledCell(String.valueOf(returnEntity.getReturnStatus()), font)));
            ...
            dateParagraph.add("\n There is no Returned Order Details found on " + format.format(LocalDateTime.now(ZoneId.of(Constants.ZONE)).minusDays(1)));
            dateTable.addCell(new Cell().add(dateParagraph)
                    .setTextAlignment(TextAlignment.LEFT)
                    .setBorder(Border.NO_BORDER));
