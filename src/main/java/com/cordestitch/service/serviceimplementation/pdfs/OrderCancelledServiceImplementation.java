...
import com.cordestitch.service.service.pdfs.OrderCancelledService;
...
@Slf4j
public class OrderCancelledServiceImplementation implements OrderCancelledService{
    ...
    @Override
    public SuccessResponse sendOrderCancelledEmail() {
        try {
            ...
            attachmentPart.setFileName("Orders_Cancelled_Report.pdf");
    ...
    public ByteArrayOutputStream generateByteArray() throws IOException {
        ...
        List<OrderItemEntity> cancelledData = orderEntityList.stream()
                .flatMap(orders -> orders.getOrderItemEntities().stream())
        ...
        dateParagraph.add("Canceled Orders Report").setFont(companyFont);
        dateParagraph.add("\n");
        if (!cancelledData.isEmpty()) {
            ...
            for (OrderItemEntity orderItemEntities : cancelledData) {

                OrderEntity entity = orderItemEntities.getOrderEntity();
                bookingTable.addCell(new Cell().add(createStyledCell(entity.getOrderId(),font)));
                bookingTable.addCell(new Cell().add(createStyledCell(orderItemEntities.getOrderItemId(),font)));
                bookingTable.addCell(new Cell().add(createStyledCell(format.format(entity.getOrderDate()),font)));
                bookingTable.addCell(new Cell().add(createStyledCell(String.valueOf(orderItemEntities.getOrderStatus()),font)));
            ...
            dateParagraph.add("\n There is no Canceled Order Details found on " + format.format(LocalDateTime.now(ZoneId.of(Constants.ZONE)).minusDays(1)));
            dateTable.addCell(new Cell().add(dateParagraph)
                    .setTextAlignment(TextAlignment.LEFT)
                    .setBorder(Border.NO_BORDER));
