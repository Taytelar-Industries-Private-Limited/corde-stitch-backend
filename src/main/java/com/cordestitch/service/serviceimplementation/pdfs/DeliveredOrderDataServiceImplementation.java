...
import com.cordestitch.service.service.pdfs.DeliveredOrderService;
...
@Slf4j
public class DeliveredOrderDataServiceImplementation implements DeliveredOrderService {
    ...
    @Override
    public SuccessResponse sendDeliveredOrderReportToEmail() {
        try {
            ...
            attachmentPart.setFileName("Delivered_Orders_Report.pdf");
    ...
    public ByteArrayOutputStream generateByteArray() throws IOException {
        ...
        List<OrderItemEntity> deliveredOrderData = orderEntityList.stream()
                .flatMap(order -> order.getOrderItemEntities().stream())
                .filter(orders -> Optional.ofNullable(orders.getDeliveryStatus())
                        .map(status -> status.equals(DeliveryStatus.DELIVERED))
                        .orElse(false))
        .toList();
        ...
        dateParagraph.add("Delivered Orders Report").setFont(companyFont);
        dateParagraph.add("\n");
        if (!deliveredOrderData.isEmpty()) {
            dateParagraph.add("Report Date: ").setFont(companyFont);
            dateParagraph.add(format.format(orderEntityList.getFirst().getOrderDate())).setFont(companyFont);
            dateParagraph.add("\n");
            ...
            float[] bookingInfoColumnWidths = {120, 120, 150, 145, 120, 150, 130, 120, 130};
            ...
            for (OrderItemEntity orderItemEntity : deliveredOrderData) {
                ...
                bookingTable.addCell(new Cell().add(createStyledCell(String.valueOf(entity.getOrderStatus()),font)));
                ...
                bookingTable.addCell(new Cell().add(createStyledCell(String.valueOf(entity.getOrderItemEntities().getFirst().getDeliveryStatus()),font)));

            }

            doc.add(bookingTable);
        } else {
            dateParagraph.add("\n There is no Delivered Order Details found on " + format.format(LocalDateTime.now(ZoneId.of(Constants.ZONE)).minusDays(1)));
            dateTable.addCell(new Cell().add(dateParagraph)
                    .setTextAlignment(TextAlignment.LEFT)
                    .setBorder(Border.NO_BORDER));
